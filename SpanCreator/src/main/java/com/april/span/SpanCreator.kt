 ...
 * TextView 设置富文本样式
 */
fun <T : SpanStyle> TextView.spanText(vararg spanStyles: T?) {
    val builder = SpannableStringBuilder()
    spanStyles.forEach { ss ->
        ss?.let {
...
open class SpanStyle(
    ...
    //编辑框时，在该位置前面插入文字是否应用该样式
    private val includeStart: Boolean = false,
    //编辑框时，在该位置后面插入文字是否应用该样式
...
) : SpanStyle(text = " ", onClick = onClick)

/**
 * 换行
 */
class NewLine : SpanStyle(text = "\n")

/**
 * 文字样式注解
 */
@IntDef(
    SpanTextStyle.Normal,
    SpanTextStyle.Bold,
    SpanTextStyle.Italic,
    SpanTextStyle.BoldItalic
)
@Retention(AnnotationRetention.SOURCE)
annotation class SpanTextStyle {
    companion object {
...
@StringDef(
    SpanTextType.Normal,
    SpanTextType.Monospace,
    SpanTextType.Serif,
    SpanTextType.Sans
)
@Retention(AnnotationRetention.SOURCE)
annotation class SpanTextType {
    companion object {
